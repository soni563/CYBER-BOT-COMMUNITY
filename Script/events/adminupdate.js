        ...
        switch (logMessageType) {
            case "log:thread-admins": {
                if (logMessageData.ADMIN_EVENT == "add_admin") {
                    ...
                    if (global.configModule[this.config.name].sendNoti) api.sendMessage(`»» NOTICE «« Update user ${logMessageData.TARGET_ID} 𝗞𝗵𝘂𝘀𝗵𝗶𝗶 𝗛𝘂𝗶𝗶 𝗴𝗿𝗼𝘂𝗽 𝗺𝗲𝗶 𝗻𝗲𝘄 𝗮𝗱𝗺𝗶𝗻 𝗺𝗶𝗹𝗶😁🫵🏾`, threadID, async (error, info) => {
                    ...
                    if (global.configModule[this.config.name].sendNoti) api.sendMessage(`»» NOTICE «« Update user ${logMessageData.TARGET_ID} 𝗕𝗼𝗵𝗼𝘁 𝗯𝘂𝗿𝗮 𝗵𝘂𝗮 \n 𝐀𝐝𝐦𝐢𝐧 𝐬𝐞 𝐡𝐚𝐭𝐚 𝐝𝐢𝐲𝐚 𝐠𝐲𝐚 🥺🥺💔|`, threadID, async (error, info) => {
                        if (global.configModule[this.config.name].autoUnsend) {
                            await new Promise(resolve => setTimeout(resolve, global.configModule[this.config.name].timeToUnsend * 1000));
                            return api.unsendMessage(info.messageID);
                        } else return;
